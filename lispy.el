...
otherwise call `lispy-out-forward' and return nil."
  (interactive "p")
  (if (dewdrops-comment-or-string-p)
...
(defun lispy--keybinding-beyond-lispy ()
  "Return the ??"
  (let* ((lispy-mode nil)
         (keys (this-single-command-keys)))
    (or (key-binding keys t)
        (key-binding (lispy--fallback-translate-input keys) t))))

(defun lispy--fallback-translate-input (keys)
  "Stolen from yasnippet's code."
  (let ((retval [])
        (i 0))
    (while (< i (length keys))
      (let ((j i)
  ...
  (let ((disable (cdr (assoc :disable blist))))
    `(lambda ,(help-function-arglist def)
       ...
       (let (cmd)
         ...
         (cond
          ((or (bound-and-true-p edebug-active)
               (string-match "blg-" (symbol-name major-mode)))
           (call-interactively (lispy--keybinding-beyond-lispy)))
          ...
          (t
           (let ((cmd (lispy--keybinding-beyond-lispy)))
...
(let ((map lispy-mode-map))
  ...
  (when lispy-use-ctrl-digits
    (define-key map (kbd "C-c C-1") 'lispy-describe-inline)
    (define-key map (kbd "C-c C-2") 'lispy-arglist-inline)
    (define-key map (kbd "C-c C-3") 'lispy-out-forward)
    (define-key map (kbd "C-c C-4") 'lispy-x)
    (define-key map (kbd "C-c C-7") 'lispy-cursor-down)
    (define-key map (kbd "C-c C-8") 'lispy-parens-down)
    (define-key map (kbd "C-c C-9") 'lispy-out-forward-newline))
  ;; ——— locals: navigation ———————————————————
